...
export function load_server_config(): ServerConfig {
	const config: ServerConfig = {
		...
		// Analytics settings
		cleanup_interval_minutes: parseInt(process.env.CLEANUP_INTERVAL_MINUTES || "60", 10),
		session_timeout_minutes: parseInt(process.env.SESSION_TIMEOUT_MINUTES || "30", 10),
		alert_threshold_visitors_per_minute: parseInt(process.env.ALERT_THRESHOLD || "10", 10),
	};
...
function validate_config(config: ServerConfig): void {
	...
	if (config.websocket_port < 1 || config.websocket_port > 65535) {
		errors.push(`Invalid WebSocket port: ${config.websocket_port}. Must be between 1 and 65535.`);
	}
	...
	if (config.session_timeout_minutes < 1) {
		errors.push(`Session timeout must be at least 1 minute, got: ${config.session_timeout_minutes}`);
	}
	
	if (config.cleanup_interval_minutes < 1) {
		errors.push(`Cleanup interval must be at least 1 minute, got: ${config.cleanup_interval_minutes}`);
	}
...
export function log_config(config: ServerConfig): void {
	...
	console.log(`   WebSocket Server: ws://${config.host}:${config.websocket_port}`);
	...
	console.log(`   Cleanup Interval: ${config.cleanup_interval_minutes} minutes`);
	console.log(`   Alert Threshold: ${config.alert_threshold_visitors_per_minute} visitors/minute`);
} 